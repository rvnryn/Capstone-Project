...
async def restore(encrypted_data, password, session, user, activity_type="restore backup", filename="uploaded file"):
    try:
        ...
        with engine.begin() as conn:
            # First, disable foreign key checks to avoid constraint issues during restore
            try:
            ...
            for table_name, records in backup_data.items():
                ...
                # Use TRUNCATE instead of DROP TABLE for safer restoration
                # TRUNCATE preserves table structure and is faster
                try:
                    conn.execute(text(f"TRUNCATE TABLE {table_name} RESTART IDENTITY CASCADE;"))
                    print(f"[Restore] Truncated table: {table_name}")
                except Exception as e:
                    # If TRUNCATE fails (table doesn't exist), we'll try to create it
                    print(f"[Restore] Table {table_name} might not exist, will attempt to create: {e}")
                ...
                if not df.empty:
                    try:
                        if pk_col and pk_col in df.columns:
                            ...
                            print(f"[Restore] Restored {len(df2)} records to {table_name}")
                        else:
                            df.to_sql(table_name, conn, if_exists="replace", index=False)
                            print(f"[Restore] Restored {len(df)} records to {table_name}")
                    except Exception as e:
                        print(f"[Restore] Error restoring table {table_name}: {e}")
                        raise HTTPException(status_code=500, detail=f"Failed to restore table {table_name}: {str(e)}")
