...
export function useNavigation(): NavigationState & {
  ...
  const getDeviceType = useCallback(
    (width: number, height: number, isTouchDevice: boolean): DeviceType => {
    ...
    if (typeof window !== "undefined") {
      setTimeout(() => {
        window.dispatchEvent(
          new CustomEvent("nav-toggle", { detail: { isOpen: open } })
        );
      }, 0);
    }
  }, []);
        ...
        setTimeout(() => {
          window.dispatchEvent(
            new CustomEvent("nav-toggle", {
              detail: { isOpen: newState.isMenuOpen },
            })
          );
        }, 0);
      }
  ...
  const closeMenu = useCallback(() => {
    ...
    if (typeof window !== "undefined") {
      setTimeout(() => {
        window.dispatchEvent(
          new CustomEvent("nav-toggle", { detail: { isOpen: false } })
        );
      }, 0);
    }
  }, []);
  ...
  const openMenu = useCallback(() => {
    ...
    if (typeof window !== "undefined") {
      setTimeout(() => {
        window.dispatchEvent(
          new CustomEvent("nav-toggle", { detail: { isOpen: true } })
        );
      }, 0);
    }
  }, []);
...
export const navigationUtils = {
  ...
  getResponsiveClass: (
    classes: Partial<Record<ScreenSize, string>>,
