...
export default function ReportSales() {
  ...
  const [pastSalesData, setPastSalesData] = useState<SalesItem[]>([]);
  ...
  useEffect(() => {
    async function fetchSales() {
      try {
        const res = await axios.get("/api/report_sales");
  ...
  // Fetch past sales when date is filtered
  useEffect(() => {
    async function fetchPastSales() {
      if (reportDate) {
        try {
          const res = await axios.get("/api/report_past_sales_log");
          setPastSalesData(
  ...
  const dataSource = useMemo(() => {
    if (reportDate) {
      return pastSalesData.filter((item) => item.report_date === reportDate);
    }
    if (period !== "all") {
      return salesData.filter((item) => matchesPeriod(item.report_date));
    }
    return salesData;
  }, [salesData, pastSalesData, reportDate, period]);
  ...
  const uniqueReportDates = useMemo(() => {
    const dates = [...salesData, ...pastSalesData]
      .map((item) => item.report_date)
      .filter((date) => !!date);
    return [...new Set(dates)];
  }, [salesData, pastSalesData]);
  ...
  }, [dataSource, searchQuery, reportDate, categoryFilter]);
  ...
  const groupedSales = useMemo(() => {
    ...
    filteredSales.forEach((item) => {
      ...
      groups[item.name].totalRevenue += item.quantity * item.unitPrice;
    });
    return Object.values(groups);
  }, [filteredSales]);

  // Add sorting to grouped sales
  ...
  const values = [
    ["Item Name", "Quantity Sold", "Unit Price", "Total Revenue"],
              ...
              <section className="bg-black rounded-lg xs:rounded-xl sm:rounded-2xl lg:rounded-3xl shadow-2xl p-1 xs:p-2 sm:p-4 md:p-6 lg:p-8 xl:p-12 w-full overflow-hidden">
                    ...
                    {(searchQuery ||
                      ...
                      <div className="mt-3 xs:mt-4 pt-3 xs:pt-4 border-t border-gray-700/30">
                        <div className="flex justify-center">
                          <button
                            onClick={handleClear}
                            className="flex items-center gap-1.5 xs:gap-2 text-red-400 hover:text-red-300 bg-red-500/10 hover:bg-red-500/20 px-3 xs:px-4 py-1.5 xs:py-2 rounded-md xs:rounded-lg border border-red-500/20 hover:border-red-500/30 transition-all duration-200 text-xs xs:text-sm font-medium"
                          >
                            <span>🗑️</span>
                ...
                <section className="bg-gray-800/30 backdrop-blur-sm rounded-md xs:rounded-lg sm:rounded-xl lg:rounded-2xl shadow-2xl border border-gray-700/50 overflow-hidden custom-scrollbar">
                        ...
                        sortedSales.map((item, index) => (
                          ...
                          >
                            ...
                            <div className="grid grid-cols-2 gap-2 xs:gap-3 mb-2">
                              ...
                              <div className="bg-yellow-500/10 rounded-md xs:rounded-lg p-1.5 xs:p-2 border border-yellow-500/20">
                                <p className="text-yellow-400 text-2xs xs:text-xs font-medium mb-0.5 truncate">
                                  Unit Price
                                </p>
                                <p className="text-white text-sm xs:text-base font-bold">
                                  ₱{item.unitPrice}
                  ...
                  <div className="hidden md:block lg:hidden overflow-x-auto">
                    <table className="table-auto w-full text-sm text-left border-collapse min-w-[500px]">
                      <thead className="bg-gradient-to-r from-gray-800/80 to-gray-900/80 backdrop-blur-sm sticky top-0 z-10">
                        <tr>
                          {[
                            { key: "name", label: "Item" },
