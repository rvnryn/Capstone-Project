...
const Report_UserActivity = () => {
  ...
  const cachedLogs =
    typeof window !== "undefined"
      ...
      : null;
  const displayLogs =
    isCompletelyOffline && cachedLogs ? JSON.parse(cachedLogs) : logs;
  ...
  const exportToExcel = async () => {
    ...
    metaCell.value = `Generated: ${new Date().toLocaleString(
      "en-US"
    )} | Total Activities: ${sortedActivity.length}`;
    ...
    sortedActivity.forEach((item, index) => {
      ...
      worksheet.addRow([
        ...
        item.description,
        actionType,
        new Date(item.activity_date).toLocaleString("en-US"),
      ]);
