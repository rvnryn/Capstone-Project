...
export default function InventoryPage() {
  ...
  const batchDateOptions = useMemo(() => {
    ...
    return Object.entries(batchMap).map(([date]) => ({
      date,
    }));
  }, [inventoryData]);
          ...
          <div className="max-w-full xs:max-w-full sm:max-w-4xl md:max-w-5xl lg:max-w-6xl xl:max-w-7xl 2xl:max-w-full mx-auto w-full">
            <article className="bg-gradient-to-br from-gray-900/95 to-black/95 backdrop-blur-sm rounded-xl xs:rounded-2xl sm:rounded-3xl shadow-2xl border border-gray-800/50 p-2 xs:p-3 sm:p-4 md:p-6 lg:p-8 xl:p-10 2xl:p-12 w-full">
              <header className="flex flex-col space-y-3 xs:space-y-4 sm:space-y-5 md:space-y-6 mb-4 xs:mb-5 sm:mb-6 md:mb-8">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 xs:gap-3 sm:gap-4 md:gap-6">
                  <div className="flex items-center gap-2 xs:gap-3 sm:gap-4">
                    <div className="relative">
                      ...
                      <div className="relative bg-gradient-to-br from-yellow-400 to-yellow-500 p-1.5 xs:p-2 sm:p-3 rounded-full">
                        <MdInventory className="text-black text-sm xs:text-base sm:text-lg md:text-xl lg:text-2xl xl:text-3xl 2xl:text-4xl" />
                      </div>
                    </div>
                    <div>
                      ...
                      <p className="text-gray-400 text-xs xs:text-sm sm:text-base mt-0.5 xs:mt-1">
                        Manage your complete inventory stock
                      </p>
                    </div>
                  </div>
                  <nav
                  ...
                  >
                    {["Owner", "General Manager", "Store Manager"].includes(
                      ...
                      <button
                        onClick={() => router.push(routes.addInventory)}
                ...
                <section
                  className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-1.5 xs:gap-2 sm:gap-3 md:gap-4"
                  aria-label="Inventory statistics"
                >
                  <article className="bg-gradient-to-br from-blue-500/10 to-blue-600/10 border border-blue-500/20 rounded-lg xs:rounded-xl p-2 xs:p-3 sm:p-4">
                    <div className="flex items-center justify-between">
                      ...
                      <FiEye className="text-blue-400 text-sm xs:text-base sm:text-lg md:text-xl lg:text-2xl" />
                    </div>
                  </article>
                  ...
                  <article className="bg-gradient-to-br from-red-500/10 to-red-600/10 border border-red-500/20 rounded-lg xs:rounded-xl p-2 xs:p-3 sm:p-4">
                    <div className="flex items-center justify-between">
                      <div>
                      ...
                      <FiTrendingDown className="text-red-400 text-sm xs:text-base sm:text-lg md:text-xl lg:text-2xl" />
                    </div>
                  </article>
                  <article className="bg-gradient-to-br from-yellow-500/10 to-yellow-600/10 border border-yellow-500/20 rounded-lg xs:rounded-xl p-2 xs:p-3 sm:p-4">
                    <div className="flex items-center justify-between">
                      ...
                      <MdWarning className="text-yellow-400 text-sm xs:text-base sm:text-lg md:text-xl lg:text-2xl" />
                    </div>
                  </article>
                      ...
                      {isLoading ? (
                        ...
                        sortedData.map((item: any, index) => (
                          <tr
                            ...
                            onClick={() => {
                              router.push(routes.ViewInventory(item.id));
                            }}
                          >
                                  ...
                                  <>
                                    <button
                                      ...
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        router.push(
                                          routes.UpdateInventory(item.id)
                                        );
                                      }}
